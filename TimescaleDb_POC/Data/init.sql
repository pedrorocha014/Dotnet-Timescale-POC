...
CREATE MATERIALIZED VIEW IF NOT EXISTS error_log_hourly
...
CREATE INDEX IF NOT EXISTS idx_error_log_hourly_stats_bucket ON error_log_hourly (bucket DESC);
CREATE INDEX IF NOT EXISTS idx_error_log_hourly_stats_exception_type ON error_log_hourly (exception_type);
CREATE INDEX IF NOT EXISTS idx_error_log_hourly_stats_bucket_exception_type ON error_log_hourly (bucket DESC, exception_type);

-- Adicionar policy para refresh automático do continuous aggregate
-- Esta policy irá atualizar o aggregate a cada hora
SELECT add_continuous_aggregate_policy('error_log_hourly',
...
INSERT INTO error_log (time, message, exception_type) VALUES
    ...
    (NOW() - INTERVAL '2 hours', 'Erro de timeout 2', 'TimeoutException'),
    ...
    (NOW() - INTERVAL '1 hour', 'Erro de autenticação 2', 'AuthenticationException'),
    (NOW() - INTERVAL '1 hour', 'Erro de autenticação 3', 'AuthenticationException'),
